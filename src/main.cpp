...
unsigned int nStakeMinAge = 1 * 24 * 60 * 60; // 24 hour
unsigned int nStakeMaxAge = 2 * 24 * 60 * 60; //48 hour
...
const string strMessageMagic = "PumpGroupCoin Signed Message:\n";
...
int64_t GetProofOfStakeReward(int64_t nCoinAge, int64_t nFees)
{
    ...
    int64_t nSubsidy = nCoinAge * 33 / (365 * 33 + 8) * nRewardCoinYear / COIN;
...
bool CBlock::AcceptBlock()
{
    ...
    if (nBits != GetNextTargetRequired(pindexPrev, IsProofOfStake()) && nHeight > FORK_HEIGHT)
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "PumpGroupCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "PumpGroupCoin - Starts now!";
        CTransaction txNew;
        txNew.nTime = 1477481567;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime = 1477481567;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce = 1354621;
		if(fTestNet)
        {
            block.nNonce = 1354621;
        }
		
		uint256 nhashMerkleRoot = uint256("0xc5df3b9e1b4bcc709f32c6d6384f249d13199166b5f3e130c51aaa9ffaa9be64");
